...
// Only the 2 core functions remain: room/workshop booking & uploading the list of
// learners on mandatory courses. Faculty Directory & CSAT moved to a shared component,
// the shared TrainerRatingsDirectory (visible to all 6 roles); the Calendar tab was dropped because
// it duplicated the in-person class schedule already in "Teaching Classes & Live QR".
  ...
  return (
    <>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <h1>Teaching Schedule &amp; Practice Workshop</h1>
            <Badge tone="rail" icon="ti-building">Room Booking &amp; Batch Upload</Badge>
          </div>
          <p>
            Book a classroom / practice workshop for an in-person class, and batch-upload the learner list for mandatory courses.
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, borderBottom: '1px solid var(--line)', paddingBottom: 8, flexWrap: 'wrap' }}>
        {[
          { id: 'ROOM_BOOKING', label: 'Room Booking', icon: 'ti-building' },
          { id: 'BATCH_UPLOAD', label: 'Batch Upload The Mandatory Course Learner List', icon: 'ti-users-plus' },
        ].map((tab) => (
          <button
            key={tab.id}
      ...
      {activeTab === 'ROOM_BOOKING' && (() => {
        const filteredVenues = meetingRoomsAndLabs.filter((v) => {
          const isLab = v.id.includes('lab') || v.name.toLowerCase().includes('workshop') || v.name.toLowerCase().includes('lab');
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}>
                ...
                <div style={{ position: 'relative', flex: '1 1 280px', minWidth: 220 }}>
                  ...
                  <input
                    ...
                    placeholder="Search by room, workshop, location, equipment..."
                    value={roomSearch}
                ...
                <div style={{ minWidth: 200 }}>
                  <select
                    ...
                    style={{
                      ...
                      fontSize: 13,
                      borderRadius: 8,
                      background: roomTypeFilter !== 'ALL' ? 'var(--blue-soft)' : 'var(--paper)',
                      borderColor: roomTypeFilter !== 'ALL' ? '#005BAA' : 'var(--line)',
                      color: roomTypeFilter !== 'ALL' ? 'var(--blue)' : 'var(--ink)',
                      fontWeight: roomTypeFilter !== 'ALL' ? 700 : 500,
                    }}
                    value={roomTypeFilter}
                    onChange={(e) => setRoomTypeFilter(e.target.value)}
                  >
                    <option value="ALL">All room / workshop types</option>
                    <option value="LAB">🏪 Practice Workshop (Store Lab)</option>
                    <option value="ROOM">🏢 Conference &amp; Training Rooms</option>
                  </select>
                </div>
              </div>
              ...
              {(roomSearch || roomTypeFilter !== 'ALL') && (
                <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px dashed var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {roomSearch && (
                      <span className="badge" style={{ background: 'var(--blue-soft)', color: 'var(--blue-soft-text)', border: '1px solid #BFDBFE', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                        Search term: <strong>"{roomSearch}"</strong>
                        <i className="ti ti-x" style={{ cursor: 'pointer' }} onClick={() => setRoomSearch('')} />
                      </span>
                    )}
                    {roomTypeFilter !== 'ALL' && (
                      <span className="badge" style={{ background: 'var(--blue-soft)', color: 'var(--blue-soft-text)', border: '1px solid #BFDBFE', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                        Room type: <strong>{roomTypeFilter === 'LAB' ? 'Practice Workshop' : 'Conference Room'}</strong>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--ink-soft)' }}>
                    Found <strong>{filteredVenues.length}</strong> / {meetingRoomsAndLabs.length} venues
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-2" style={{ gap: 16 }}>
              {filteredVenues.length === 0 ? (
                <div className="card empty-state" style={{ gridColumn: '1 / -1', padding: '32px 16px' }}>
                  ...
                  No matching venue or practice workshop found.
                </div>
              ) : (
                filteredVenues.map((v) => (
                  <div key={v.id} className="card card-pad">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                      <div>
                        <div style={{ fontWeight: 800, fontSize: 15, color: 'var(--ink)' }}>{v.name}</div>
                    ...
                    <div style={{ background: 'var(--paper-sunken)', borderRadius: 8, padding: '10px 14px', marginBottom: 12 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink-soft)', marginBottom: 4 }}>Equipment &amp; Facilities Available:</div>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        {v.equipment.map((eq, i) => (
                          <span key={i} style={{ background: 'var(--paper-raised)', padding: '2px 8px', borderRadius: 4, border: '1px solid var(--line)', fontSize: 12 }}>
                            • {eq}
                          </span>
                        ))}
                      </div>
                    ...
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink-soft)', marginBottom: 4 }}>Upcoming Bookings:</div>
                      {bookingsForRoom(v.id).length === 0 ? (
                        <div style={{ fontSize: 12, color: 'var(--ink-faint)' }}>No bookings scheduled.</div>
                      ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                          {bookingsForRoom(v.id).map((b) => (
                            <div key={b.id} style={{ fontSize: 12, color: 'var(--ink-soft)' }}>
      ...
      >
        {selectedVenue && (
          <div>
            ...
            />
            {reserveError && (
              <div style={{ marginTop: 10, fontSize: 13, color: 'var(--rust)' }}>
