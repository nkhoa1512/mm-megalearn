...
<head>
<meta charset="UTF-8">
<title>MM MegaLearn — SRS & Functional Specification (Full Codebase Aligned)</title>
<style>
  @page {
    size: A4;
...
<div class="cover-page">
  ...
  <div class="cover-body">
    <div class="doc-type">Software Requirements Specification & Functional Specification (SRS / FSD)</div>
    <div class="doc-title">TÀI LIỆU ĐẶC TẢ YÊU CẦU NGHIỆP VỤ & CHỨC NĂNG HỆ THỐNG</div>
    <div class="doc-subtitle">
      Đặc tả chi tiết chuẩn hóa 100% theo mã nguồn Front-end Mockup và Bảng yêu cầu thực tế của MM Mega Market Vietnam:
      Cơ cấu Tổ chức Nhánh đôi (Dual Hierarchy), 7 Vai trò Người dùng (RBAC), Trạng thái Nhân viên chuẩn HRIS (Active, Inactive, Transfer, New Joiner), 
      Động cơ Gán 10 Phạm vi, Quản lý Chi phí & Ngân sách Đào tạo (Cost Tracking), Hồ sơ Năng lực Chi tiết (Talent Profile 4 Tabs), 
      Chỉ định & Đề cử Khóa học (Nominate Modal), Khảo sát Post-Training L1 (CSAT), Thiết lập Cam kết Action Plan & Đánh giá Hành vi L3 Sau 3-6 Tháng (Post-Training Survey Modal),
      Khảo thí Đánh giá Rút đề Ngẫu nhiên, Trình phát Đa định dạng (bao gồm YouTube), Lớp Thực hành ILT & Điểm danh Live QR, 
      Đặt Phòng Thực hành Chống trùng lịch, Lộ trình 70-20-10, AI Learning Hub & Báo cáo ROI Kirkpatrick.
    </div>

    <div class="meta-grid">
      <div class="meta-item">
        <div class="meta-label">MÃ TÀI LIỆU:</div>
        <div class="meta-val">SRS-FSD-MMVN-MEGALEARN-2026-V6.3</div>
      </div>
      <div class="meta-item">
        <div class="meta-label">PHIÊN BẢN (VERSION):</div>
        <div class="meta-val">6.3 (Full Codebase & Action Plan Verified)</div>
      </div>
      <div class="meta-item">
      ...
      <div class="meta-item">
        ...
        <div class="meta-val" style="color:#166534;">✓ Đồng bộ hoàn toàn 100% Mã nguồn & Yêu cầu</div>
      </div>
      <div class="meta-item">
...
<ul>
  ...
  <li><strong>Quản trị viên Đào tạo (L&OD Admin)</strong>: Soạn thảo khóa học kéo thả (Course Builder), quản lý ngân hàng câu hỏi & import CSV, cấu hình luật gán tự động (Auto-assignment rules), quản trị phòng thực hành & lịch đặt phòng, xem báo cáo toàn diện Kirkpatrick ROI và xuất dữ liệu CSV / PDF.</li>
</ul>
...
<ul>
  ...
  <li><strong>BR-029 (Hệ số Thưởng Điểm XP Gamification)</strong>:
    Hoàn thành bài học: $+20\text{ XP}$; Thi đạt điểm tối đa $100\%$: $+100\text{ XP}$; Điểm danh Live QR Lớp ILT: $+150\text{ XP}$; Học liên tục: $+1\text{ Streak Day}$.
  </li>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        - Tạo mới và cấu hình khóa học theo cấu trúc <code>Course &gt; Module &gt; Lesson</code>. Mỗi lần lưu thay đổi trên khóa học đã tồn tại tự động tăng phiên bản (<code>v1.0 &rarr; v1.1</code>...) và ghi một dòng vào <strong>Version History</strong> hiển thị ngay trong Course Builder (người cập nhật, ngày giờ, ghi chú), giữ toàn bộ lịch sử theo thứ tự mới nhất trước.<br>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        - Hỗ trợ 4 định dạng câu hỏi: <code>SINGLE_CHOICE</code>, <code>MULTIPLE_CHOICE</code>, <code>TRUE_FALSE</code>, <code>SHORT_ANSWER</code> (trả lời ngắn tự luận — chấm tự động bằng cách so khớp với danh sách đáp án chấp nhận được, không phân biệt hoa/thường, hỗ trợ nhiều đáp án thay thế).<br>
        - Bộ tiền kiểm duyệt CSV: Phát hiện và bỏ qua các dòng lỗi (thiếu câu hỏi, thiếu đáp án, ít hơn 2 lựa chọn — hoặc thiếu đáp án chấp nhận với câu SHORT_ANSWER), đếm chính xác số dòng lỗi và thông báo chi tiết cho Admin, không làm mất dữ liệu âm thầm.
      </td>
    </tr>
    <tr>
      ...
      <td>
        ...
        - Chấm điểm tức thì, ghi nhận số lần thi (Attempt), hiển thị đáp án theo cấu hình và cập nhật tiến độ 70/30.<br>
        - <strong>Xáo trộn hiển thị</strong>: 2 công tắc độc lập trong cấu hình khóa học — <code>randomizeQuestions</code> (thứ tự câu hỏi) và <code>randomizeAnswers</code> (thứ tự đáp án) — áp dụng mỗi lượt thi, tách biệt khỏi việc rút tập con câu hỏi ở BR-022.
      </td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><strong>FR-TRN-003</strong></td>
      <td>Đăng ký Hàng loạt Học viên (Batch Enroll)</td>
      <td>
        - Tab riêng trong Training Ops cho phép Admin/Trainer tải lên danh sách học viên theo lớp thực hành để ghi danh hàng loạt, thay thế việc học viên phải tự đăng ký từng người cho các lớp ILT đặc thù.
      </td>
    </tr>
  </tbody>
</table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        - Bộ chỉ số KPI cá nhân và danh sách khóa học lọc theo 4 Tab (All, Mandatory, Optional, Completed).<br>
        - <strong>Learning Hours</strong>: Tổng số giờ học lũy kế tính theo thời lượng ước tính từng khóa học nhân với tiến độ thực tế (<code>estimatedHours &times; progressPercent</code>), hiển thị tại Dashboard và Learning History.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td><strong>FR-LRN-004</strong></td>
      <td>Tìm kiếm Khóa học (Catalog Search)</td>
      <td>
        - Thanh tìm kiếm tại Training Catalog lọc theo mã khóa, tiêu đề hoặc kỹ năng (skill); kết hợp bộ lọc Domain/Topic theo danh mục nghiệp vụ.
      </td>
    </tr>
  </tbody>
</table>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td><strong>FR-MGR-005</strong></td>
      <td>Phân tích Khoảng cách Kỹ năng Đội ngũ (Team Skill Gap Matrix)</td>
      <td>
        - So sánh năng lực yêu cầu cho vị trí kế nhiệm mục tiêu với điểm năng lực thực tế của từng nhân viên cấp dưới, hiển thị dạng lưới thẻ theo từng kỹ năng (không phải biểu đồ radar) kèm chỉ số Net Gap tổng hợp.<br>
        - Nút <strong>Assign Developmental Course</strong> 1-chạm để gán ngay khóa học bù đắp khoảng cách vừa phát hiện.
      </td>
    </tr>
  </tbody>
</table>
...
<h3>4. Bảng <code>approval_requests</code> (Đơn Đăng ký & Phê duyệt Khóa học)</h3>
<table>
  <thead>
    <tr>
      <th style="width:20%;">Tên Cột</th>
  ...
  <tbody>
    <tr>
      ...
      <td>VARCHAR(64)</td>
      <td>PK, NOT NULL</td>
      <td>Mã yêu cầu phê duyệt (Ví dụ: <code>req-101</code>).</td>
    </tr>
    <tr>
      <td><code>user_id</code></td>
      <td>VARCHAR(64)</td>
      <td>FK &rarr; users.id, NOT NULL</td>
      <td>Mã nhân viên nộp đơn đăng ký.</td>
    </tr>
    <tr>
      <td><code>course_id</code></td>
      <td>VARCHAR(64)</td>
      <td>FK &rarr; courses.id, NOT NULL</td>
      <td>Mã khóa học đề xuất tham gia.</td>
    </tr>
    <tr>
      <td><code>course_cost</code></td>
      <td>VARCHAR(64)</td>
      <td>NOT NULL</td>
      <td><strong>Chi phí chương trình</strong> (Ví dụ: <code>4,500,000 VND</code>).</td>
    </tr>
    <tr>
      <td><code>justification</code></td>
      <td>TEXT</td>
      <td>NOT NULL</td>
      <td>Lý do / Giải trình nhu cầu phát triển năng lực.</td>
    </tr>
    <tr>
      <td><code>status</code></td>
      <td>ENUM</td>
...
<h3>5. Bảng <code>learning_enrollments</code> (Tiến trình Học tập)</h3>
<table>
  <thead>
    <tr>
      <th style="width:20%;">Tên Cột</th>
  ...
  <tbody>
    <tr>
      ...
      <td>VARCHAR(64)</td>
      <td>PK, NOT NULL</td>
      <td>Khóa chính Enrollment (Ví dụ: <code>enr-1042-fsh</code>).</td>
    </tr>
    <tr>
      <td><code>user_id</code></td>
      <td>VARCHAR(64)</td>
      <td>FK &rarr; users.id, NOT NULL</td>
      <td>Mã nhân viên học tập.</td>
    </tr>
    <tr>
      <td><code>course_id</code></td>
      <td>VARCHAR(64)</td>
      <td>FK &rarr; courses.id, NOT NULL</td>
      <td>Mã khóa học tham gia.</td>
    </tr>
    <tr>
      <td><code>status</code></td>
      <td>ENUM</td>
      <td>NOT NULL</td>
      <td><code>NOT_STARTED</code>, <code>IN_PROGRESS</code>, <code>COMPLETED</code>, <code>FAILED</code>, <code>OVERDUE</code>.</td>
    </tr>
    <tr>
      <td><code>progress_percent</code></td>
      <td>INT</td>
      <td>DEFAULT 0</td>
      <td>Tiến độ từ $0\%$ đến $100\%$ tính theo công thức chuẩn 70/30.</td>
    </tr>
    <tr>
      <td><code>final_score</code></td>
      <td>INT</td>
      <td>NULL</td>
      <td>Điểm thi cao nhất đạt được ($\%$).</td>
    </tr>
    <tr>
      <td><code>completed_at</code></td>
      <td>TIMESTAMP</td>
      <td>NULL</td>
      <td>Thời điểm hoàn thành toàn bộ điều kiện khóa học.</td>
    </tr>
  </tbody>
</table>

<h3>6. Bảng <code>assessment_attempts</code> (Nhật ký Thi Bất biến)</h3>
<table>
  <thead>
    <tr>
      <th style="width:20%;">Tên Cột</th>
  ...
  <tbody>
    <tr>
      ...
      <td>VARCHAR(64)</td>
      <td>PK, NOT NULL</td>
      <td>Khóa chính bản ghi thi (Ví dụ: <code>att-98124</code>).</td>
    </tr>
    <tr>
      <td><code>enrollment_id</code></td>
      <td>VARCHAR(64)</td>
      <td>FK &rarr; learning_enrollments.id</td>
      <td>Mã bản ghi tiến trình tương ứng.</td>
    </tr>
    <tr>
      <td><code>attempt_number</code></td>
      <td>INT</td>
      <td>NOT NULL</td>
      <td>Lần thi thứ mấy (1, 2, 3...).</td>
    </tr>
    <tr>
      <td><code>score_percent</code></td>
      <td>INT</td>
      <td>NOT NULL</td>
      <td>Điểm số đạt được ($\%$).</td>
    </tr>
    <tr>
      <td><code>passed</code></td>
      <td>BOOLEAN</td>
      <td>NOT NULL</td>
      <td>Kết quả Đạt (<code>true</code>) hoặc Không đạt (<code>false</code>).</td>
    </tr>
    <tr>
      <td><code>duration_seconds</code></td>
      <td>INT</td>
      <td>NOT NULL</td>
      <td>Thời gian thực tế làm bài (giây).</td>
    </tr>
    <tr>
      <td><code>submitted_at</code></td>
      <td>TIMESTAMP</td>
      <td>DEFAULT CURRENT_TIMESTAMP</td>
      <td>Thời điểm nộp bài (Append-only, không cho phép sửa/xóa).</td>
    </tr>
  </tbody>
</table>
...
<!-- SECTION 6 -->
<h1>6. YÊU CẦU PHI CHỨC NĂNG (NFR) & KẾ HOẠCH BÀN GIAO</h1>

<h2>6.1. Yêu cầu Phi Chức năng Cốt lõi (Non-Functional Requirements)</h2>
<ul>
  ...
  <li><strong>NFR-UI-001 (Bảng màu Thương hiệu)</strong>: Màu chủ đạo xanh lá cây (thương hiệu BigC/MM, token <code>--rail</code>) cho điều hướng và hành động chính; 3 màu phụ theo đúng yêu cầu: xanh dương (<code>--blue</code>, thông tin), đỏ (<code>--rust</code>, quá hạn/lỗi/chặn), đen (<code>--ink</code>, văn bản chính) — cộng thêm hổ phách (<code>--amber</code>, đang xử lý/chờ) cho trạng thái trung gian. Không dùng màu làm kênh thông tin duy nhất, luôn kèm badge/label.</li>
</ul>

<h2>6.2. Kế hoạch Lộ trình Triển khai (Release Roadmap)</h2>
<table>
  <thead>
    <tr>
  ...
  <tbody>
    <tr>
      ...
      <td>Thiết kế Kiến trúc, UI Mockup & Đặc tả SRS/FSD</td>
...
<div style="margin-top: 24px; padding: 12px; background: #f0fdf4; border: 1px solid #86efac; border-radius: 8px; font-size: 11px; color: #166534;">
  <strong>XÁC NHẬN ĐỐI CHIẾU TOÀN DIỆN (100% CODEBASE & REQUIREMENT FULLY SYNCHRONIZED):</strong><br>
  Tài liệu đặc tả này bao gồm đầy đủ tính năng Khảo sát L1 CSAT, Cam kết Action Plan 90 ngày & Đánh giá Hành vi L3 Sau 3-6 Tháng (FR-MGR-004),
  Chỉ định khóa học Nominate Modal (FR-MGR-002), Hồ sơ Talent Profile Modal 4 Tabs (FR-PRF-001), và đồng bộ chính xác 100% với mã nguồn React thực tế.
  Vòng đối chiếu gần nhất bổ sung 5 mục có trong yêu cầu công ty nhưng trước đó chưa được đặc tả — Team Skill Gap Matrix (FR-MGR-005),
  Đăng ký hàng loạt học viên (FR-TRN-003), Xáo trộn hiển thị câu hỏi/đáp án (FR-ASSESS-002), Bảng màu thương hiệu (NFR-UI-001), Tìm kiếm khóa học (FR-LRN-004) —
  đồng thời cập nhật 3 tính năng vừa được lập trình bổ sung vào mã nguồn để khớp yêu cầu công ty: Version History thực tế cho khóa học (FR-CRS-001),
  loại câu hỏi trả lời ngắn SHORT_ANSWER (FR-ASSESS-001), và theo dõi Learning Hours (FR-LRN-001).
</div>
