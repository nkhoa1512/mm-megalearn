...
export default function LearnerLearningPaths({ initialTab = 'CURRENT' }) {
  ...
  const { currentUser, getUserRoadmapTabs, requestRoadmapPromotion, levelAdvanceRequestsFor, enrollCourse } = useCourseStore();
  const [activeTab, setActiveTab] = useState(initialTab);
  const [requestState, setRequestState] = useState(null); // null | 'ok' | 'not-ready'

  const roadmap = getUserRoadmapTabs(currentUser);
  ...
  // Trang này được mount ở nhiều route khác nhau tùy role (/learner/paths cho
  // Learner, /my-learning-path dùng chung cho 5 role còn lại) — luôn điều
  // hướng qua route xem khóa học DÙNG CHUNG /my-learning/:courseId (giống
  // MyLearning.jsx) thay vì hard-code /learner/courses/:courseId, để không vỡ
  // link khi Manager/Trainer/HRBP/UserAdmin/SysAdmin bấm "Vào Học Ngay".
  ...
  function joinTrack(track) {
    track.milestones.forEach(({ course, completed }) => {
      if (!completed) enrollCourse(course.id, currentUser);
    });
  }
      ...
      {activeTab === 'CURRENT' && (
        <div className="card card-pad">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
            <div style={{ fontWeight: 800, fontSize: 15 }}>Định Biên Level {roadmap.level}: {levelDef.shortVi}</div>
        ...
        <div className="card card-pad">
          {!roadmap.nextLevel ? (
            <div className="empty-state">
              ...
              <p>Bạn đã ở cấp bậc cao nhất (Level 1) — không còn Lộ trình kế cận.</p>
            </div>
          ) : (
            <>
              <div
                style={{
                  display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 8, marginBottom: 18,
                  background: roadmap.succession.locked ? '#FEF2F2' : '#F0FDF4',
              ...
              >
                ...
                <span style={{ fontSize: 13, fontWeight: 600 }}>
                  {roadmap.succession.locked
                    ...
                    : `Bạn đã hoàn thành định biên Level ${roadmap.level}. Lộ trình kế cận Level ${roadmap.nextLevel} đã được mở khóa!`}
                </span>
              </div>
      ...
      {activeTab === 'SELF_PROPOSED' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ background: 'var(--rail-soft)', color: 'var(--rail-soft-text)', padding: '12px 16px', borderRadius: 8, fontSize: 12.5 }}>
            ...
            Các lộ trình chuyên đề mở rộng ngoài định biên — bạn tự chọn theo định hướng phát triển bản thân, hoặc do Quản lý trực tiếp giao thêm.
          </div>
          {roadmap.selfProposed.tracks.length === 0 ? (
            <div className="card empty-state">
              <i className="ti ti-mood-empty" />
              <p>Chưa có track nào phù hợp cấp bậc hiện tại của bạn.</p>
            </div>
          ) : (
              ...
              <div key={track.id} className="card card-pad">
                ...
                <div style={{ marginTop: 12, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {track.milestones.map(({ course, completed }) => (
                    <Badge key={course.id} tone={completed ? 'sage' : 'slate'} icon={completed ? 'ti-check' : 'ti-book-2'}>
                      {course.code}
                    </Badge>
                  ))}
                </div>
      ...
      {activeTab === 'RECOMMENDED' && (
        <div>
          ...
          {roadmap.recommended.length === 0 ? (
            <div className="card empty-state">
              <i className="ti ti-mood-empty" />
              <p>Không có gợi ý mới — bạn đã hoàn thành phần lớn nội dung phù hợp.</p>
            </div>
          ) : (
